...
const BASE_T =4294967296.0;

void ROUND( F, a, b, c, d, k, s, i ) {
 a = ( a + F( b, c, d ) + x[ k ] + 
   ( int ) ( BASE_T * fabs( sin( ( double ) i ) ) ) ); 
 a = ( a << s ) | ( a >> ( 32 - s ) ); 
 a += b;
}

List<int> md5_initial_hash = [
  0x67452301,
  0xefcdab89,
  0x98badcfe,
  0x10325476
...
// void md5_block_operate( List<int> input, 
//                 List<int> hash[ MD5_RESULT_SIZE ] )
// {
...
{  
  ...
  context.block_len = 0;
  context.hash = ( int * ) 
   malloc( context.hash_len * sizeof( int ) );
  memcpy( context.hash, md5_initial_hash, 
   context.hash_len * sizeof( int ) );
