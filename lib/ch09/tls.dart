...
class CipherSuite
{
  ...
  void (*bulk_encrypt)( const List<int> plaintext,
                        const int plaintext_len,
                        unsigned char ciphertext[],
                        void *iv,
                        const List<int> key );
  void (*bulk_decrypt)( const List<int> ciphertext,
                        const int ciphertext_len,
                        unsigned char plaintext[],
                        void *iv,
                        const List<int> key );
  void (*new_digest)( digest_ctx *context );
  int (*aead_encrypt)( const List<int> plaintext,
                       const int plaintext_len,
                       const List<int> addldata,
                       const int addldata_len,
                       unsigned char ciphertext[],
                       void *iv,
                       const List<int> key );
  int (*aead_decrypt)( const List<int> ciphertext,
                       const int ciphertext_len,
                       const List<int> addldata,
                       const int addldata_len,
                       unsigned char plaintext[],
...
List<int> random_type
=List.filled(RANDOM_LENGTH, 0);
...
class TLSParameters
{
  ...
  List<int>                  unread_buffer;
  int                   unread_length;
  int                   session_id_length;
  List<int>          session_id[ MAX_SESSION_ID_LENGTH ];
  int                   support_secure_renegotiation;
  List<int>          client_verify_data[ VERIFY_DATA_LEN ];
  List<int>          server_verify_data[ VERIFY_DATA_LEN ];
}
...
class ServerHello
{
  ...
  TlsRandom            random;
  int     session_id_length;
  List<int>     session_id[ 32 ]; // technically, this len should be dynamic.
  intt    cipher_suite;
...
static CipherSuite suites[MAX_SUPPORTED_CIPHER_SUITE] =
{
...
{ 
  ...
  if ( active_suite->bulk_encrypt || active_suite->aead_encrypt )
  {
    unsigned char *encrypted_buffer = malloc( send_buffer_size +
      active_suite->IV_size + ( active_suite->aead_encrypt ? active_suite->hash_size : 0 ) );
    int plaintext_len;
                             ...
                             TLSParameters *parameters )
{
  ...
  update_digest( &sha_digest, parameters->client_random, RANDOM_LENGTH );
  update_digest( &sha_digest, parameters->server_random, RANDOM_LENGTH );
  update_digest( &sha_digest, message, message_len );
  finalize_digest( &sha_digest );
...
void compute_handshake_hash( TLSParameters *parameters, unsigned char *handshake_hash )
{
  ...
  memcpy( &tmp_sha256_handshake_digest, &parameters->sha256_handshake_digest, 
    sizeof( digest_ctx ) );

  tmp_sha256_handshake_digest.hash = ( unsigned int * ) malloc( SHA256_BYTE_SIZE );
  memcpy( tmp_sha256_handshake_digest.hash, parameters->sha256_handshake_digest.hash, 
    SHA256_BYTE_SIZE );

  finalize_digest( &tmp_sha256_handshake_digest );

  memcpy( handshake_hash, tmp_sha256_handshake_digest.hash, 
    SHA256_BYTE_SIZE );
