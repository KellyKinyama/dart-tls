...
void ecdsa_sign( EllipticCurve params, 
         Huge private_key,
         List<int> hash, 
         int hash_len, 
         DsaSignature signature )
{
  ...
  Huge k;
  Point X=Point(set_huge(  0 ), set_huge(  0 ));
  Huge z;

  // This should be a random number between 0 and n-1
 k  =load_huge(  K, sizeof( K ) );
 // k=Huge.fromList(K);

  // set_huge( X.x, 0 );
  // set_huge( X.y, 0 );
// X=Point(Huge.from(0),Huge.from(0));

  // copy_huge( X.x, params.G.x );
  // copy_huge( X.y, params.G.y );
  params.G=Point(set_huge(  0 ), set_huge(  0 ));

  multiply_point( X, k, params.a, params.p );

  signature.r =set_huge(  0 );
...
int ecdsa_verify( EllipticCurve params,
         Point public_key,
         List<int> hash,
         int hash_len,
         DsaSignature signature )
{
  ...
  // z=load_huge( hash, 
  //   ( hash_len * 4 ) < params.n.size ) ? ( hash_len * 4 ) : params.n.size );
z=load_huge(hash,(hash_len*4)<params.n.size?(hash_len*4):params.n.size);
  ...
  match = compare( G.x, signature.r )==0?1:0;
...
int main( int argc, List<int> argv )
{
  // ECC parameters
  int P = [ 
 ...
 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
  ...
  // key
  int w = [ 0xDC, 0x51, 0xD3, 0x86, 0x6A, 0x15, 0xBA, 0xCD, 0xE3, 
  ...
  digest_ctx ctx;
   ...
   curve.p =load_huge(  P, sizeof( P ) );
  curve.a =set_huge(  3 );
  curve.a.sign = 1;
  curve.b=load_huge(  b, sizeof( b ) );
  curve.G.x=load_huge(  gx, sizeof( gx ) );
  curve.G.y=load_huge(  gy, sizeof( gy ) );
  curve.n=load_huge(  q, sizeof( q ) );

  // Generate new public key from private key �w� and Point �G�
  A.d=load_huge(  w, sizeof( w ) );
  A.Q.x=set_huge(  0 );
   A.Q.y=set_huge( 0 );
  A.Q.x=copy_huge(  curve.G.x );
  A.Q.y=copy_huge(  curve.G.y );
