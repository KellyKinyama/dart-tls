...
const SHA1_RESULT_SIZE =5;
const SHA1_BYTE_SIZE =SHA1_RESULT_SIZE * 32;

const SHA256_RESULT_SIZE =8;
const SHA256_BYTE_SIZE =SHA256_RESULT_SIZE * 32;
...
void sha1_finalize( List<int> padded_block, int length_in_bits )
{
  padded_block[ SHA1_BLOCK_SIZE - 4 ] = ( length_in_bits & 0xFF000000 ) >> 24;
  padded_block[ SHA1_BLOCK_SIZE - 3 ] = ( length_in_bits & 0x00FF0000 ) >> 16;
  padded_block[ SHA1_BLOCK_SIZE - 2 ] = ( length_in_bits & 0x0000FF00 ) >> 8;
...
void new_sha1_digest( digest_ctx context )
{
  ...
  context.input_len = 0;
  context.block_len = 0;
  context.hash = List.filled(context.hash_len * 32 , 0) ;
  memcpy( context.hash, sha1_initial_hash, 
  context.hash_len * 32 );

  context.hash=List.from(sha1_initial_hash) ; 
  context.block=List.filled(DIGEST_BLOCK_SIZE, '\0'.codeUnits[0]);
...
void new_sha256_digest( digest_ctx context )
{
  ...
  context.input_len = 0;
  context.block_len = 0;
  context.hash = ( List<int> ) malloc( context.hash_len * 
    32 );
