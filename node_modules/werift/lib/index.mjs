...
var { uint16be, uint24be, buffer, array, uint8, string } = types;
var Extension = {
  type: uint16be,
...
var ServerHelloDone = class _ServerHelloDone {
  ...
  static deSerialize(buf) {
    return new _ServerHelloDone(
      ...Object.values(decode15(buf, _ServerHelloDone.spec))
    );
  }
  serialize() {
    const res = encode16(this, _ServerHelloDone.spec).slice();
    return Buffer.from(res);
  }
  toFragment() {
    const body = this.serialize();
...
var DtlsSocket = class {
  ...
  extractSessionKeys(keyLength2, saltLength2) {
    ...
    const { clientKey, serverKey, clientSalt, serverSalt } = decode21(
      keyingMaterial,
      {
        clientKey: types22.buffer(keyLength2),
        serverKey: types22.buffer(keyLength2),
        clientSalt: types22.buffer(saltLength2),
        serverSalt: types22.buffer(saltLength2)
      }
    );
    if (this.sessionType === SessionType.CLIENT) {
      return {
...
function parseMessage(data, integrityKey) {
  ...
  const [messageType, length] = jspack4.Unpack(
    "!HHI",
  ...
  for (let pos = HEADER_LENGTH; pos <= data.length - 4; ) {
    const [attrType, attrLen] = jspack4.Unpack(
      "!HH",
...
var Message = class extends AttributeRepository {
  ...
  get bytes() {
    ...
    for (const attrName of this.attributesKeys) {
      ...
      data = Buffer.concat([
        data,
        Buffer.from(jspack4.Pack("!HH", [attrType, attrLen])),
        v,
        ...[...Array(padLen)].map(() => Buffer.from("\0"))
      ]);
    }
...
var setBodyLength = (data, length) => {
  return Buffer.concat([
    data.slice(0, 2),
    Buffer.from(jspack4.Pack("!H", [length])),
    data.slice(4)
  ]);
};
...
var TurnProtocol = class _TurnProtocol {
  ...
  handleChannelData(data) {
    const [channel, length] = jspack5.Unpack("!HH", data.slice(0, 4));
  ...
  async sendData(data, addr) {
    ...
    const header = jspack5.Pack("!HH", [channel.number, data.length]);
...
var Connection = class {
  ...
  async restart() {
    ...
    this.promiseGatherCandidates = void 0;
  }
  resetNominatedPair() {
  ...
  async getCandidates(addresses, timeout = 5) {
    ...
    if (stunServer) {
      const stunPromises = this.protocols.map(
        (protocol) => new Promise(async (r, f) => {
    ...
    const extraCandidates = [...await Promise.allSettled(candidatePromises)].filter(
      (v) => v.status === "fulfilled"
    ).map((v) => v.value).filter((v) => typeof v !== "undefined");
...
var ntpTime = () => {
  const now2 = performance.timeOrigin + performance.now() - Date.UTC(1900, 0, 1);
...
var RTCDtlsTransport = class _RTCDtlsTransport {
  constructor(config, iceTransport, router, localCertificate, srtpProfiles = []) {
    this.config = config;
...
function parsePacket2(data) {
  ...
  const [sourcePort, destinationPort, verificationTag] = jspack6.Unpack(
    "!HHL",
    data
  );
  ...
  while (pos + 4 <= data.length) {
    const [chunkType, chunkFlags, chunkLength] = jspack6.Unpack(
      "!BBH",
      data.slice(pos)
    );
...
function serializePacket(sourcePort, destinationPort, verificationTag, chunk) {
  const header = Buffer.from(
    jspack6.Pack("!HHL", [sourcePort, destinationPort, verificationTag])
  );
...
var OutgoingSSNResetRequestParam = class _OutgoingSSNResetRequestParam {
  ...
  get bytes() {
    ...
    return Buffer.concat([
      data,
      ...this.streams.map((stream) => Buffer.from(jspack7.Pack("!H", [stream])))
    ]);
  }
  static parse(data) {
    const [requestSequence, responseSequence, lastTsn] = jspack7.Unpack(
      "!LLL",
      data
    );
    const stream = range4(12, data.length, 2).map(
      (pos) => jspack7.Unpack("!H", data.slice(pos))[0]
    );
...
var StreamAddOutgoingParam = class _StreamAddOutgoingParam {
  ...
  get bytes() {
    return Buffer.from(
      jspack7.Pack("!LHH", [this.requestSequence, this.newStreams, 0])
    );
  }
  static parse(data) {
    const [requestSequence, newStreams] = jspack7.Unpack("!LHH", data);
...
var ReconfigResponseParam = class _ReconfigResponseParam {
  ...
  get bytes() {
    return Buffer.from(
      jspack7.Pack("!LL", [this.responseSequence, this.result])
    );
  }
  static parse(data) {
    const [requestSequence, result] = jspack7.Unpack("!LL", data);
      ...
      case InitChunk.type:
        {
          ...
          let cookie = Buffer.from(jspack8.Pack("!L", [time]));
      ...
      case CookieEchoChunk.type:
        {
          ...
          const stamp = jspack8.Unpack("!L", cookie)[0];
...
var RTCSctpTransport = class {
        ...
        case DATA_CHANNEL_OPEN:
          {
            ...
            this.dataChannelQueue.push([
              channel,
              WEBRTC_DCEP,
              Buffer.from(jspack9.Pack("!B", [DATA_CHANNEL_ACK]))
            ]);
            await this.dataChannelFlush();
          }
          break;
  ...
  dataChannelOpen(channel) {
    ...
    const data = jspack9.Pack("!BBHLHH", [
      DATA_CHANNEL_OPEN,
      channelType,
      priority,
      reliability,
...
var RtpRouter = class {
  ...
  registerRtpReceiverBySsrc(transceiver, params) {
    ...
    params.encodings.filter((e) => e.ssrc != void 0).forEach((encode22, i) => {
      this.registerRtpReceiver(transceiver.receiver, encode22.ssrc);
      ...
      if (encode22.rtx) {
        this.registerRtpReceiver(transceiver.receiver, encode22.rtx.ssrc);
      }
    });
...
var RTCRtpSender = class {
  ...
  type = "sender";
  kind;
  ssrc = jspack10.Unpack("!L", randomBytes8(4))[0];
  rtxSsrc = jspack10.Unpack("!L", randomBytes8(4))[0];
  streamId = uuid4.v4();
  trackId = uuid4.v4();
  onReady = new Event();
  onRtcp = new Event();
...
var log34 = debug("werift:packages/webrtc/src/peerConnection.ts");
var RTCPeerConnection = class extends EventTarget {
  ...
  buildOfferSdp() {
    ...
    this.transceivers.filter((t) => !description.media.find((m) => m.rtp.muxId === t.mid)).forEach((transceiver) => {
    ...
    if (this.sctpTransport && !description.media.find((m) => m.kind === "application")) {
      ...
      if (this.sctpTransport.mid == void 0) {
  ...
  createTransport(srtpProfiles = []) {
    ...
    iceTransport.onIceCandidate.subscribe((candidate) => {
      if (!this.localDescription) {
        log34("localDescription not found when ice candidate was gathered");
        return;
      }
      if (!candidate) {
  ...
  async setLocalDescription(sessionDescription) {
    ...
    await this.gatherCandidates().catch((e) => {
      log34("gatherCandidates failed", e);
    });
    if (description.type === "answer") {
      this.connect().catch((err5) => {
        log34("connect failed", err5);
  ...
  async addIceCandidate(candidateMessage) {
    ...
    if (iceTransport) {
      ...
      log34("iceTransport not found", candidate);
    }
  }
  async connect() {
    log34("start connect");
    const res = await Promise.allSettled(
      this.dtlsTransports.map(async (dtlsTransport) => {
        ...
        await iceTransport.start().catch((err5) => {
          log34("iceTransport.start failed", err5);
          throw err5;
        });
        ...
        await dtlsTransport.start().catch((err5) => {
          log34("dtlsTransport.start failed", err5);
          throw err5;
        });
    ...
    let transports = remoteSdp.media.map((remoteMedia, i) => {
      ...
      if (["audio", "video"].includes(remoteMedia.kind)) {
        let transceiver = this.transceivers.find(
          (t) => matchTransceiverWithMedia(t, remoteMedia)
        );
        if (!transceiver) {
  ...
  validateDescription(description, isLocal) {
    if (isLocal) {
      if (description.type === "offer") {
        ...
        if (!["have-remote-offer", "have-local-pranswer"].includes(
          this.signalingState
        )) {
      ...
      if (description.type === "offer") {
        ...
        if (!["have-local-offer", "have-remote-pranswer"].includes(
          this.signalingState
        )) {
          throw new Error("Cannot handle answer in signaling state");
        }
    ...
    if (["answer", "pranswer"].includes(description.type || "")) {
      ...
      if (!isEqual2(offerMedia, answerMedia)) {
  ...
  updateIceGatheringState() {
    ...
    log34("iceGatheringStateChange", newState);
  ...
  updateIceConnectionState() {
    ...
    log34("iceConnectionStateChange", newState);
  ...
  setSignalingState(state) {
    log34("signalingStateChange", state);
  ...
  setConnectionState(state) {
    log34("connectionStateChange", state);
